  ...
  return (
    ...
    >
      ...
      <Box>
        <Typography
          variant="h6 "
          sx={{ fontSize: { xs: "8px", sm: "12px", md: "20px" } }}
        >
          {value}
        </Typography>
        <Typography
...
const Hero = () => {
  ...
  const [heroData, setHeroData] = useState({
    title: "Be a Leader in your field - Grow with Lcurve",
    description: "",
    statsValue: "10k",
    statsTitle: "Assistant Student",
  });

  useEffect(() => {
    const fetchHeroData = async () => {
      try {
        const response = await staticPageService.getByPageType("HERO");
        if (response?.data) {
          setHeroData({
            title:
              response.data.title ||
              "Be a Leader in your field - Grow with Lcurve",
            description: response.data.description || "",
            statsValue: response.data.email || "10k", // Reusing email field for simple stats or adding new ones if schema allows
            statsTitle: response.data.phoneNumber || "Assistant Student",
          });
        }
      } catch (error) {
      ...
      {heroData.title.split(" - ").map((part, index) => (
        <React.Fragment key={index}>
          {index === 0 ? (
            <>
              {part.split("Leader")[0]}
              <HighlightText>Leader</HighlightText>
              {part.split("Leader")[1]}
            </>
          ) : (
            part
  ...
  const imageContent = (
    ...
    >
      ...
      <FloatingCard
        title={heroData.statsTitle}
        value={heroData.statsValue}
        style={{
          position: "absolute",
          bottom: "25%",
          left: { xs: "-20%", sm: "-30%" },
  ...
  return (
    <Container maxWidth="lg" sx={{ pb: { xs: 4, sm: 6, md: 8 } }}>
      ...
      >
        ...
        <Grid>{imageContent}</Grid>
      </Grid>
      <Box sx={{ display: { xs: "flex", sm: "none" } }}>{buttonContent}</Box>
    </Container>
  );
};
